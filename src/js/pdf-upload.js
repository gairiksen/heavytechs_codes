...
const _form = document.querySelector('[data-s3-uppy-upload-type="pdf"]');
...
const uppy = Uppy({
  ...
  restrictions: {
    maxFileSize: 209715200, // Limit size to 2 MB on the javascript side
...
const createPhoto = (imageUrl) => {
  ...
  const pageCategory = _form.dataset.s3UppyPageCategory;
  const itemCategory = _form.dataset.s3UppyItemCategory;
  const userID = _form.dataset.s3UppyUserId;

  // Create model for this user with s3 image url
  return apiFetch('/api/pdf', {
    body: JSON.stringify({ pdfDetails: { direct_url: imageUrl, photo_type: photoType, object_uuid: objectUuid, page_category: pageCategory, item_category: itemCategory, user_id: userID } })
  });
};
